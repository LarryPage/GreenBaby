...
- (void) dealloc
{
	...
	for(NSNumber *tmpSourceID in temporarySounds)
	{
		...
		ALuint srcID = [tmpSourceID unsignedIntValue];
...
- (void) deleteTemporarySource
{
	...
	ALuint tmpSourceID = [[temporarySounds objectAtIndex:0] unsignedIntValue];
...
- (void) setVolume:(ALfloat)newVolume
{
	...
	{
		//tmpSourceID is the source ID for the temporary sound
		alSourcef([tmpSourceID unsignedIntValue], AL_GAIN, volume);
	}
}
...
- (void) setPitch:(ALfloat)newPitch
{
	...
	{
		//tmpSourceID is the source ID for the temporary sound
		alSourcef([tmpSourceID unsignedIntValue], AL_PITCH, pitch);
	}
}
@end

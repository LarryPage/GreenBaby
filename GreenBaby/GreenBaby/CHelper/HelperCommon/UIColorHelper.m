...
// Pick the color that is 180 degrees度数 away in hue
...
// Pick two colors more colors such that all three are equidistant等距 on the color wheel车轮
...
- (NSString *)closestColorNameFor: (const char *) aColorDatabase {
    ...
    for (name = bestPos-1; (name != strchr(name, ',')); --name)//lxc更改于2012-3-18
		;
	++name;
...
+ (void)hue:(CGFloat)h saturation:(CGFloat)s brightness:(CGFloat)v toRed:(CGFloat *)pR green:(CGFloat *)pG blue:(CGFloat *)pB {
	...
	if (s == 0.0f) {
		// Achromatic color无色的: there is no hue
		r = g = b = v;
	} else {
		// Chromatic color彩色的: there is a hue
		if (h == 360.0f) h = 0.0f;
...
+ (void)populateColorNameCache {
	...
	for (const char* entry = colorNameDB; (entry = strchr(entry, ',')); ) {
		...
		int hex, increment;
		if (sscanf(++h, "%x%n", &hex, &increment) != 1) {name=nil; break;} // thanks Curtis Duhn

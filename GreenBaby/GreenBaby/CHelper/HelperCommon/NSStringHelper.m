...
- (NSString*)stringByURLEncodingStringParameterWithEncoding:(NSStringEncoding)stringEncoding {
	...
	if (!stringEncoding) stringEncoding=NSUTF8StringEncoding;
	//!  @  $  &  (  )  =  +  ~  `  ;  '  :  ,  /  ?
...
- (CGFloat)heightWithPadding:(float)padding minimumHeight:(float)minimumHeight maximumHeight:(float)maximumHeight  fontSize:(float)fontSize {
    CGSize size = [self adjustSizeWithFont:[UIFont systemFontOfSize:fontSize] constrainedToSize:CGSizeMake(300.0f, 99999999.0f)];
...
- (CGSize)adjustSizeWithFont:(UIFont *)font constrainedToSize:(CGSize)size{
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 7.0) {
        ...
        CGRect labelRect = [self boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:font} context:nil];
        ...
        return CGSizeMake(ceilf(labelRect.size.width), ceilf(labelRect.size.height));
    }
    else {
...
//Hztojpy.c
//First Letter
//void GenerateAllJianPin(char strSrc[],char** strDst, int *nNum,int nSrcLen);
//将len个字节的UTF8格式的转换成GBK格式存放在temp预先申请好的缓冲区中
//void Utf8ToGBK(const char* utf8,int len,char *temp);


/*
 *  CPinyin.h
 *  Chinese CPinyin First Letter
 *
 *  Created by Li Xiang Cheng on 4/21/10.
...
@implementation NSString (NSString_ChineseToPinyin)
...
- (NSString *) pinyinFromChiniseString
{
	...
	NSData * gb2312_data = [self dataUsingEncoding:enc];
...
- (char) sortSectionTitle
{
	...
	{	
		if( ([self characterAtIndex:0] > 64 &&  [self characterAtIndex:0] < 91 ) || 
		   ([self characterAtIndex:0] > 96 &&  [self characterAtIndex:0] < 123 ) )
		{
			cLetter = [self characterAtIndex:0];
		}
		else
			cLetter = pinyinFirstLetter((unsigned short)[self characterAtIndex:0] );
...
//	const char *pstrSrc = [self cStringUsingEncoding:NSUTF8StringEncoding];
//	NSInteger iUTF8Len = [self lengthOfBytesUsingEncoding:NSUTF8StringEncoding];
//	char szGBKSrc[50] = { 0 };
//	Utf8ToGBK(pstrSrc,iUTF8Len,szGBKSrc);
//	char *pstrDst = nil;
//	int nstrLen = 0,GBKLen = 0;;
//	for(;GBKLen < 50;GBKLen++)
//	{
//		if(!szGBKSrc[GBKLen])
//			break;
//	}
//	GenerateAllJianPin(szGBKSrc,&pstrDst,&nstrLen,GBKLen);
    
    char letter = pinyinFirstLetter([self characterAtIndex:0]);
    //NSString * alphabet = [[NSString alloc] initWithFormat:@"%c",letter];
	return [[NSString stringWithFormat:@"%c",letter] uppercaseString];
}
...
- (NSData *)base64Data {
	...
	MeBase64DecodeData(inputData, inputDataSize, outputData, &outputDataSize);//decode the data
	return [[NSData alloc] initWithBytes:outputData length:outputDataSize];//create a NSData object from the decoded data
}

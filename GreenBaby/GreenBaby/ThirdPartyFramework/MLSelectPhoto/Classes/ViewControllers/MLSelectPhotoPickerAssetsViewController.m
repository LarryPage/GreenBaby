...
- (UIButton *)previewBtn{
    if (!_previewBtn) {
        ...
        previewBtn.titleLabel.font = CFont(17);
...
- (void)pickerCollectionViewDidCameraSelect:(MLSelectPhotoPickerCollectionView *)pickerCollectionView{
    ...
    if (self.selectAssets.count >= KPhotoShowMaxCount) {
        [self.view showMessageWithText:[NSString stringWithFormat:@"选择的图片个数不能大于%@",@(KPhotoShowMaxCount)]];
        return ;
    }
...
- (void)preview{
    ...
    browserVc.maxCount=self.maxCount;
    browserVc.doneText=self.doneText;
    [browserVc setValue:@(YES) forKeyPath:@"isEditing"];
    browserVc.photos = [NSMutableArray arrayWithArray:self.selectAssets];
    browserVc.doneAssets = [NSMutableArray arrayWithArray:self.selectAssets];
    browserVc.deleteAssets = [NSMutableDictionary dictionary];
...
// 选择相片预览区域就会调用,modify by lxc
- (void) pickerCollectionViewDidSelected:(MLSelectPhotoPickerCollectionView *) pickerCollectionView previewIdx:(NSInteger)index{
    MLSelectPhotoBrowserViewController *browserVc = [[MLSelectPhotoBrowserViewController alloc] init];
    browserVc.maxCount=self.maxCount;
    browserVc.doneText=self.doneText;
    [browserVc setValue:@(YES) forKeyPath:@"isEditing"];
    browserVc.photos = [NSMutableArray arrayWithArray:self.collectionView.dataArray];
    browserVc.doneAssets = [NSMutableArray arrayWithArray:self.selectAssets];
    
    NSMutableDictionary *deleteAssets=[NSMutableDictionary dictionary];
    for (NSInteger i = 0; i < browserVc.photos.count; i++) {
        MLSelectPhotoAssets *asset = browserVc.photos[i];
        if (![browserVc.doneAssets containsObject:asset]) {
            [deleteAssets setObject:@YES forKey:[NSString stringWithFormat:@"%@",@(i)]];
        }
    }
    browserVc.deleteAssets = deleteAssets;

...
- (IBAction)previewBtn:(id)sender{
    ...
    if ([self.collectionViewDelegate respondsToSelector:@selector(pickerCollectionViewDidSelected: previewIdx:)]) {
        [self.collectionViewDelegate pickerCollectionViewDidSelected:self previewIdx:btn.tag];
    }
}
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //modify by lxc
    cellImgView.previewBtn.tag=indexPath.row;
    [cellImgView.previewBtn addTarget:self action:@selector(previewBtn:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if(indexPath.item == 0 && self.topShowPhotoPicker){
        ...
        if (self.isRecoderSelectPicker) {
            for (MLSelectPhotoAssets *asset in self.selectAsstes) {
                if ([asset isKindOfClass:[MLSelectPhotoAssets class]] && [asset.asset.defaultRepresentation.url isEqual:[(MLSelectPhotoAssets *)self.dataArray[indexPath.item] asset].defaultRepresentation.url]) {

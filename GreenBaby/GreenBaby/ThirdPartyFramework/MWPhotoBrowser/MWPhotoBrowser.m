...
- (void)viewDidLoad {
    ...
    leftBtn.frame = CGRectMake(0, 0, 44.0f, 44.0f);
    leftBtn.tag = LEFT_BAR_BUTTON;
    [leftBtn setImage:[UIImage imageNamed:@"Btn_Back_white.png"] forState:UIControlStateNormal];
    [leftBtn setImage:[UIImage imageNamed:@"Btn_Back_white_hl"] forState:UIControlStateHighlighted];
...
#pragma mark Action modify by lxc
...
#pragma clang diagnostic push
            ...
            [[UIApplication sharedApplication] setStatusBarStyle:DefaultStatusBarStyle animated:animated];//modify by lxc
        }
    }
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    if (result == MFMailComposeResultFailed) {
        [UIAlertController alert:NSLocalizedString(@"Email failed to send. Please try again.", nil) title:NSLocalizedString(@"Email", nil) bTitle:NSLocalizedString(@"Dismiss", nil)];
    }
    [self dismissViewControllerAnimated:YES completion:nil];
}

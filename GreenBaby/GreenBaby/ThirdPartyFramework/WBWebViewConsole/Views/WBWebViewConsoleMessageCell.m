...
+ (CGFloat)rowHeightOfDataObject:(WBWebViewConsoleMessage *)message tableView:(UITableView *)tableView
{
    ...
    CGFloat messageHeight = ceil([message.message adjustSizeWithFont:font constrainedToSize:CGSizeMake(width, CGFLOAT_MAX)].height);
    ...
    if (caller.length)
    {
        ...
        CGFloat callerHeight = ceil([message.caller adjustSizeWithFont:font constrainedToSize:CGSizeMake(width, CGFLOAT_MAX)].height);
        height += callerHeight;
    }

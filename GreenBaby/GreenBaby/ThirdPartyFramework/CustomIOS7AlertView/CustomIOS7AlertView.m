...
- (id)initWithParentView: (UIView *)_parentView
{
    ...
    if (self) {
        ...
        //buttonTitles = @[@"Close"];
        //modify by lxc
        buttonTitles = [[NSArray alloc] initWithObjects:@"Close", nil];
    }
    return self;
}
...
- (id)init
{
    [[NSNotificationCenter defaultCenter]addObserver:self
                                            selector:@selector(textFieldMessageChange:)
                                                name:@"CustomIOS7Alert_TextFieldMessageChange"
                                              object:nil];
...
- (void)show
{
    ...
    // Attached to the top most window (make sure we are using the right orientation):
        ...
        //modify by lxc
        [[UIApplication sharedApplication].delegate.window insertSubview:self aboveSubview:[UIApplication sharedApplication].delegate.window.rootViewController.view];
    }
...
{
    NSLog(@"Button Clicked! %@, %@", @(buttonIndex), @([alertView tag]));
    [self close];
}
...
- (UIView *)createContainerView
{
    ...
    //modify by lxc 不让居中，稍微靠上44,可避免双拼键盘
    UIView *dialogContainer = [[UIView alloc] initWithFrame:CGRectMake((screenWidth - dialogWidth) / 2, (screenHeight - dialogHeight) / 2-(IS_IPHONE_5?44:84), dialogWidth, dialogHeight)];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if (i == buttonTitles.count - 1){
            self.optionLastBtn = closeButton;
...
//modify by lxc
- (void)textFieldMessageChange:(NSNotification *)n{
    UITextField *txf = n.object;
    NSString *reason = [txf.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    self.optionLastBtn.enabled = reason.length > 0;
}
@end

	...
	if (numberOfLevels > 0) {
		...
		for (int i = 0; i < numberOfLevels; i++) {
			...
			if (i < [self.tableViews count]) {
                ...
                tableView.backgroundColor=MKRGBA(245,245,245,255);//modify by lxc
...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.dataSource respondsToSelector:@selector(multiTablesView:level:canEditRowAtIndexPath:)]) {
        canEdit = [self.dataSource multiTablesView:self level:[self indexOfTableView:tableView] canEditRowAtIndexPath:indexPath];
    }
    return canEdit;
}
...
-(void)pushNextTableView:(UITableView*)tableView {
    ...
    //modify by lxc
    if (indexOfTableView+1>=[self numberOfLevels]) {
        return;
    }

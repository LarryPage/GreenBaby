...
@property (nonatomic, strong) UIImageView *logoIV;//modify by lxc
...
- (id)initWithScrollView:(UIScrollView *)scrollView {
    ...
    //modify by lxc
    self.logoIV=[[UIImageView alloc] initWithFrame:CGRectMake(0, -20, 150, 20)];
    self.logoIV.image=[UIImage imageNamed:@"pullLogo"];
    ...
    titleLabel.text = NSLocalizedString(@"下拉刷新...", @"Pull to refresh...");
...
- (void)layoutSubviews {
    self.logoIV.frame=CGRectMake((self.superview.bounds.size.width-150)/2, -20, 150, 20);
...
- (UIImageView *)arrow {
    if(!arrow) {
        ...
        arrow.frame = CGRectMake(0, 20, ArrowImageWidth, ArrowImageHeight);
...
- (UIImage *)arrowImage {
    CGRect rect = CGRectMake(0, 0, ArrowImageWidth, ArrowImageHeight);
    ...
    CGContextClipToMask(context, rect, [[UIImage imageNamed:@"grayArrow"] CGImage]);
    CGContextFillRect(context, rect);
...
- (void)setState:(SVPullToRefreshState)newState {
    ...
    switch (newState) {
        ...
        case SVPullToRefreshStateTriggered:
            titleLabel.text = NSLocalizedString(@"松开后刷新...", @"Release to refresh...");
            [self rotateArrow:M_PI hide:NO];
            break;
            
        case SVPullToRefreshStateLoading:
            titleLabel.text = NSLocalizedString(@"正在载入...", @"Loading...");

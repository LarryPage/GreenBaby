...
//
//  Created by LiXiangCheng on 13-12-25.
...
-(void)drawRect:(CGRect)rect{
    ...
    CGSize titleSize = [self.text adjustSizeWithFont:self.font  constrainedToSize:CGSizeMake(self.bounds.size.width, MAXFLOAT)];
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    ...
    //add by lxc
    if (vc.indexPathsForSelectedItems.count == vc.maximumNumberOfSelection && [_indexPathsForSelectedItems indexOfObject:asset]==NSNotFound) {
        [[TKAlertCenter defaultCenter] postAlertWithMessage:[NSString stringWithFormat:@"最多只能选择%@张图片",@(vc.maximumNumberOfSelection)]];
    }

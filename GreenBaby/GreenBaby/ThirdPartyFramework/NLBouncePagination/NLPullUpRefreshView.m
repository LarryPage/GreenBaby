...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.refreshLabel.font = CFontB(12.0);
        self.refreshLabel.textAlignment = NSTextAlignmentCenter;
        self.refreshLabel.text = REFRESH_PULL_UP_STATUS;
        
        self.refreshArrow = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"NL_arrow.png"]];
        self.refreshArrow.frame = CGRectMake(70,
                                        (floorf(REFRESHER_HEIGHT - 23) / 2),
                                        23, 23);
...
//    CLog(@"scrollView.contentSize.height:%@",@(scrollView.contentSize.height));
    if (scrollView.contentSize.height>scrollView.bounds.size.height) {
        ...
        return (scrollView.bounds.size.height - scrollView.contentInset.bottom)-scrollView.contentSize.height-REFRESHER_HEIGHT-scrollView.contentOffset.y;
    }
}

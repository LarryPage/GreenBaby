...
- (void)reloadData {
    ...
    CGFloat paddingX=(self.frame.size.width - 4 * kXHShareMenuItemWidth) / 5;
    CGFloat paddingY = 10;
    ...
    self.shareMenuPageControl.numberOfPages = (self.shareMenuItems.count / (kXHShareMenuPerRowItemCount * kXHShareMenuPerColum) + (self.shareMenuItems.count % (kXHShareMenuPerRowItemCount * kXHShareMenuPerColum) ? 1 : 0));
    self.shareMenuPageControl.currentPage = 0;
    [self.shareMenuScrollView setContentSize:CGSizeMake(((self.shareMenuItems.count / (kXHShareMenuPerRowItemCount * kXHShareMenuPerColum) + (self.shareMenuItems.count % (kXHShareMenuPerRowItemCount * kXHShareMenuPerColum) ? 1 : 0)) * CGRectGetWidth(self.bounds)), CGRectGetHeight(self.shareMenuScrollView.bounds))];
}
...
- (void)setup {
    self.autoresizingMask = UIViewAutoresizingFlexibleWidth;
     ...
     BOOL bMultiPage=_shareMenuItems.count>kXHShareMenuPerRowItemCount*kXHShareMenuPerColum;
    if (!_shareMenuScrollView) {
        UIScrollView *shareMenuScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.bounds), bMultiPage?(CGRectGetHeight(self.bounds) - kXHShareMenuPageControlHeight):CGRectGetHeight(self.bounds))];
    ...
    if (!_shareMenuPageControl && bMultiPage) {
        ...
        [shareMenuPageControl addTarget:self action:@selector(pageChange:) forControlEvents:UIControlEventValueChanged];
...
- (void)pageChange:(id)sender {
    [self.shareMenuScrollView setContentOffset:CGPointMake(self.shareMenuPageControl.currentPage*self.shareMenuScrollView.frame.size.width,0) animated:YES];
    [self.shareMenuPageControl setCurrentPage:self.shareMenuPageControl.currentPage];
}

...
{
    zlog(@"willRotate: orient=%@ #%g", @(orient), duration);
    rotating = YES;
    if(readerView)
...
- (void) willAnimateRotationToInterfaceOrientation: (UIInterfaceOrientation) orient
                                          duration: (NSTimeInterval) duration
{
    zlog(@"willAnimateRotation: orient=%@ #%g", @(orient), duration);
    if(helpController)
...
{
    zlog(@"didRotate(%d): orient=%@", rotating, @(orient));
...
- (void) setCameraCaptureMode: (UIImagePickerControllerCameraCaptureMode) mode
{
    NSAssert2(mode == UIImagePickerControllerCameraCaptureModeVideo,
              @"attempt to set unsupported value (%@)"
